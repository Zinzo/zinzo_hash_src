...
function destination_path(link)
{
  return p.join(link);
}
...
module.exports = function hash_src(options)
{
    ...
    var CustomHash = "";
    CustomHash = createHash('sha1').update(current_date + random).digest('hex');
    if(!options){
        options = {};
    }
    ...
    function get_hashes(data, base, cb)
    {
        ...
        girdle.async_loop(matches, cb, function oneach(match, next)
        {
            ...
            full_path = options.destination_path(link);
            fs_helper.fs.exists(full_path, function onres(exists)
            {
                if (!exists) {
                    if (options.verbose) {
                        console.log("WARNING: \"" + full_path + "\" does not exist. Cannot hash.");
                    }
                    return next();
                }
                fs_helper.is_dir(full_path, function onres(err, is_dir)
                {
                    if (is_dir) {
                        if (options.verbose) {
                            console.log("WARNING: \"" + full_path + "\" is a directory. Cannot hash.");
                        }
                        return next();
                    }
                    if (options.verbose) {
                        console.log("Hashing \"" + full_path + "\".");
                    }
    ...
    function rewrite(data, base)
    {
        return data.replace(options.regex, function add_hash()
        {
            ...
            props.link = props.link.split("?cbh")[0];
            return props.prefix + props.link + "?" + (options.query_name ? options.query_name + "=" : "") + CustomHash;
        });
    }
